"""
Swiggy Instamart (swiggy.com/instamart) quick-commerce scraper adapter.
"""
...
class InstamartScraper(BaseScraper):
    """Scraper adapter for Swiggy Instamart."""

    def __init__(self):
        super().__init__("Swiggy Instamart")

    async def search(self, query: str) -> ScrapeResult:
        encoded_q = urllib.parse.quote_plus(query)
        search_url = f"https://www.swiggy.com/instamart/search?custom_back=true&query={encoded_q}"
        ...
        # 2. Try HTML card parsing fallback
        if not products:
            cards = soup.select("a[href*='/instamart/item/']") or soup.select("div[data-testid='item-card']")
            for card in cards:
                try:
                    href = card.get("href", "")
                    prod_url = f"https://www.swiggy.com{href}" if href and not href.startswith("http") else (href or search_url)

                    title_el = card.select_one("div[class*='item-name']") or card.select_one("div")
                    title = title_el.get_text(strip=True) if title_el else ""
                    if not title or len(title) < 2:
                        continue

                    price_el = card.select_one("div[class*='price']") or card.select_one("span")
